pico-8 cartridge // http://www.pico-8.com
version 16
__lua__
-- d-day
-- by projectsam

function bossf(template)
  local boss = {}
  boss.x = template.x
  boss.y = template.y
  boss.dx = 0;
  boss.dy = 0;
  boss.maxspeed = template.maxspeed
  boss.acc = template.acc or 0.5
  boss.f_throttle = 8
  boss.fire = 0
  boss.tick = 1
  boss.states = template.spritedef
  boss.state = boss.states.standing
  boss.busy = false
  boss.shoot_dir = 1
  boss.hp = template.hp
  define_bounding_box(boss, 1, 2, 6, 7)
  boss.update = template.updater
  boss.shoot_func = template.shoot_func
  boss.collection = template.collection
  boss.height = template.sprh or 1
  boss.points = template.points
  function boss:shoot()
    add(self.collection, self.shoot_func(self.x, self.y, 0, 3, 1, self))
  end
  return boss
end

function cannonf(template)
  local c = {}
  c.x = template.x
  c.y = template.y
  c.vx = template.vx
  c.vy = template.vy
  c.sprite = template.sprite
  c.shoot = function()
    add(template.collection, template.shoot_func(c.x, c.y, c.vx, c.vy, 1, c))
  end
  c.f_throttle = flr(template.hertz * 30)
  c.fire = c.f_throttle
  c.hp = template.hp
  c.boomcount = template.hp
  c.points = template.points
  define_bounding_box(c, 1, 0, 7, 6)
  return c
end

function grenadef(x, y, vx, vy, dir, shooter)
  local g = {}
  g.sprite = 66
  g.x = x
  g.y = y
  g.vx = vx
  g.vy = vy * dir
  g.deaccel = 0.1
  g.dir = dir
  g.boomcount = 3
  g.shooter = shooter
  return g
end

function bulletf(x, y, vx, vy, dir, shooter)
 local b = {}
 b.shooter = shooter
 b.x = x
 b.y = y
 b.basesprite = 65
 b.sprite = 65
 b.vx = vx
 b.vy = vy
 b.range = 128
 b.dt = 0
 b.hashit = false
 define_bounding_box(b, 3, 4, 4, 7)
 sfx(5)
 return b
end

function make_smoke(x,y, vx, vy, max_radius)
  local s = {}
  s.x = x
  s.y = y
  s.vx = vx or -1 + rnd(2)
  s.vy = vy or -1 + rnd(2)
  s.radius = 0.5 + rnd(4)
  s.max_radius = max_radius or 4 + rnd(8)
  s.growth = 0.25 + rnd(0.25)
  s.alive = true
  function s:update()
    self.x = self.x + self.vx
    self.y = self.y + self.vy
    self.radius = self.radius + s.growth
    if self.radius > self.max_radius then
      self.alive = false
    end
  end
  function s:draw(offset)
    local col = 5
    if self.radius > s.max_radius / 2 then
      col = 6
    end
    circfill(self.x + offset, self.y, self.radius, col)
  end
  return s
end

function boss_update(boss)
  if abs(cam_y - boss.y) > 128 then
    return
  end
  local left = false
  local right = false
  local shoot = true
  if boss.x < 104 then
    if boss.dx >= 0 then
      right = true
    else
      left = true
    end
  else
    left = true
  end
  if boss.x < 16 then
    left = false
    right = true
  end
  update_soldier(boss, left, right, false, false, false)
  for b in all(bosses) do
    if b.y > boss.y then
      shoot = false
    end
  end
  if shoot and boss.fire == 0 then
    boss:shoot()
    boss.fire = boss.f_throttle
  end
end

function boss_update_constant(boss)
  if abs(cam_y - boss.y) > 128 then
    return
  end
  local left = false
  local right = false
  if boss.x < 104 then
    if boss.dx >= 0 then
      right = true
    else
      left = true
    end
  else
    left = true
  end
  if boss.x < 16 then
    left = false
    right = true
  end
  update_soldier(boss, left, right, false, false, false)
  if boss.fire == 0 then
    boss:shoot()
    boss.fire = boss.f_throttle
  end
end

function hitler_update(h)
  if abs(cam_y - h.y) > 128 then
    return
  end
  local shoot = true
  for b in all(bosses) do
    if b.y > h.y then
      shoot = false
    end
  end
  update_soldier(h, false, false, false, false, false)
  if shoot and h.fire == 0 then
    h:shoot()
	  for i = 0,1 do
	    add(grenades, grenadef(h.x - 4 + (i * 8), h.y + 8, -1 + rnd(2), 1.5 + rnd(1), 1, h))
	  end
	h.fire = h.f_throttle
  end
end

sprtype = {[1] = 48, [3] = 65, [4] = 64, [5] = 112, [6] = 96}
sprstates = {
     ["soldier"] = {
     	 ["standing"] = {48},
		 ["running"] = {48,48,48,48,49,49,49,49,50,50,50,50,51,51,51,51,52,52,52,52,53,53,53,53},
		 ["exploding"] = {54,54,54,55,55,55,56,56,56,57,57,57,58,58,58,59,59,59,60,60,60},
		 ["drowning"] = {38,38,38,39,39,39,40,40,40,41,41,41,42,42,42,43,43,43,44,44,44,45,45,45},
     ["shot"] = {12,12,12,13,13,13,14,14,14,15,15,15,27,27,27},
     ["hit"] = {28,28,29,29,28,28}
	},
	["nazi"] = {
	  ["standing"] = {112},
	  ["running"] = {112,112,112,112,113,113,113,113,114,114,114,114,115,115,115,115,116,116,116,116,117,117,117,117},
	  ["exploding"] = {118,118,118,119,119,119,56,56,56,57,57,57,58,58,58,59,59,59,60,60,60},
	  ["drowning"] = {96,96,96,97,97,97,98,98,98,99,99,99,100,100,100,101,101,101},
      ["shot"] = {12,12,12,13,13,13,14,14,14,15,15,15,27,27,27},
      ["hit"] = {28,28,29,29,28,28}
	},
  ["general"] = {
    ["standing"] = {90},
	["running"] = {90,90,90,90,90,91,91,91,91,91,92,91,92,92,92,92,92,92},
	["exploding"] = {106,106,106,107,107,107,56,56,56,57,57,57,58,58,58,59,59,59,60,60,60},
	["drowning"] = {96,96,96,97,97,97,98,98,98,99,99,99,100,100,100,101,101,101},
    ["shot"] = {122,122,122,123,123,123,14,14,14,15,15,15,27,27,27},
    ["hit"] = {28,28,29,29,28,28}
  },
  ["hitler"] = {
    ["standing"] = {78},
	["running"] = {78},
	["exploding"] = {78,79,79,79,79,79,79,79,109,109,109,109,109,109,109,110,110,110,110,110,110,110,110,111,111,111,111,111,111,111,112,112,112,112,112,112,112,112},
	["shot"] = {78,79,79,79,79,79,79,79,109,109,109,109,109,109,109,110,110,110,110,110,110,110,110,111,111,111,111,111,111,111,112,112,112,112,112,112,112,112},
	["hit"] = {78,79,78,79,78}
	},
  ["boom"] = {80,80,80,81,81,81,82,82,82,83,83,83,84,84,84,85,85,85,86,86,86}
}

game_modes = {
  {["name"] = "easy", ["hp"] = 8, ["grenades"] = 32, ["spawn"] = 0, ["lives"] = 20},
  {["name"] = "tougher", ["hp"] = 6, ["grenades"] = 20, ["spawn"] = 1, ["lives"] = 12},
  {["name"] = "mad", ["hp"] = 4, ["grenades"] = 12, ["spawn"] = 2, ["lives"] = 6},
  {["name"] = "brexit", ["hp"] = 1, ["grenades"] = 4, ["spawn"] = 3, ["lives"] = 2},
  {["name"] = "cheat", ["hp"] = 99, ["grenades"] = 99, ["spawn"] = 0, ["lives"] = 99}
}

bullets = {}
grenades = {}
booms = {}
player = {}
nazis = {}
cannons = {}
bosses = {}
smokes = {}
casualties = 0
lives = 0
points = 0
current_level = {}
boss_trigger = false
cheat_mode = 0

player = {}
cam_x = 0
cam_y = 0
cam_positions = {}
high_scores = {}
game_mode = game_modes[1]

levelstates = {
  {
    ["number"] = 1,
    ["offset"] = 0,
    ["nazi_spawn_rate"]= 2,
    ["overheads"]={{["sprite"] = 10,["x"] = 56, ["y"] = 88}},
    ["cannons"]={
      {["sprite"]=102, ["x"]=64, ["y"]=0, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=1, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10 },
      {["sprite"]=104, ["x"]=0, ["y"]=24, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=1, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=10 },
      {["sprite"]=105, ["x"]=120, ["y"]=24, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=1, ["hp"]=5, ["vx"] = -3, ["vy"] = 0, ["points"]=10 }
    },
    ["bosses"]={}
  },
  {
    ["number"] = 2,
    ["offset"]=128,
    ["nazi_spawn_rate"]= 3,
    ["overheads"]={
      {["sprite"] = 10,["x"] = 8, ["y"] = 256},
      {["sprite"] = 10,["x"] = 16, ["y"] = 256},
      {["sprite"] = 10,["x"] = 40, ["y"] = 256},
      {["sprite"] = 10,["x"] = 64, ["y"] = 256},
      {["sprite"] = 10,["x"] = 72, ["y"] = 256},
      {["sprite"] = 10,["x"] = 96, ["y"] = 256}
      },
    ["cannons"]={
      {["sprite"]=102, ["x"]=32, ["y"]=400, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=1, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=102, ["x"]=80, ["y"]=400, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=1, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=103, ["x"]=40, ["y"]=272, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=103, ["x"]=96, ["y"]=272, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=102, ["x"]=24, ["y"]=0, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=103, ["x"]=64, ["y"]=0, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=102, ["x"]=104, ["y"]=0, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=10},
      {["sprite"]=104, ["x"]=-4, ["y"]=24, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=10},
      {["sprite"]=105, ["x"]=124, ["y"]=12, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.5, ["hp"]=5, ["vx"] = -3, ["vy"] = 0, ["points"]=10}
    },
    ["bosses"]={}
  },
  {
    ["number"] = 3,
    ["offset"]=256,
    ["nazi_spawn_rate"]= 4,
    ["overheads"]={},
    ["cannons"]={
      {["sprite"]=103, ["x"]=56, ["y"]=384, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=7, ["vx"] = 0, ["vy"] = 3, ["points"]=20},
      {["sprite"]=104, ["x"]=0, ["y"]=408, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=20},
      {["sprite"]=102, ["x"]=128, ["y"]=456, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = -2.5, ["vy"] = -2.5, ["points"]=20},
      {["sprite"]=102, ["x"]=-8, ["y"]=456, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 2.5, ["vy"] = -2.5, ["points"]=20},
      {["sprite"]=103, ["x"]=32, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=1.5, ["hp"]=5, ["vx"] = 0, ["vy"] = 2, ["points"]=20},
      {["sprite"]=103, ["x"]=48, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=1, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=20},
      {["sprite"]=103, ["x"]=64, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=2, ["hp"]=5, ["vx"] = 0, ["vy"] = 4, ["points"]=20},
      {["sprite"]=102, ["x"]=120, ["y"]=104, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=8, ["vx"] = 0, ["vy"] = 3, ["points"]=20}
	  },
    ["bosses"]={
      {["spritedef"] = sprstates.general, ["x"] = 8, ["y"]=8, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=4, ["acc"]=1, ["updater"] = boss_update, ["hp"]=10, ["points"]=50},
      {["spritedef"] = sprstates.general, ["x"] = 112, ["y"]=20, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=3.5, ["acc"]=1, ["updater"] = boss_update, ["hp"]=8, ["points"]=50},
      {["spritedef"] = sprstates.general, ["x"] = 8, ["y"]=32, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=3, ["acc"]=1, ["updater"] = boss_update, ["hp"]=6, ["points"]=50},
      {["spritedef"] = sprstates.general, ["x"] = 112, ["y"]=44, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=3, ["acc"]=1, ["updater"] = boss_update, ["hp"]=4, ["points"]=50}
    }
  },
  {
    ["number"] = 4,
    ["offset"] = 384,
    ["nazi_spawn_rate"] = 5,
    ["overheads"] = {
      {["sprite"]=10, ["x"] = 56, ["y"]=80},
      {["sprite"]=10, ["x"] = 112, ["y"]=264}
    },
    ["cannons"]= {
	    {["sprite"]=102, ["x"]=128, ["y"]=448, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = -2.5, ["vy"] = 2.5, ["points"]=50},
      {["sprite"]=102, ["x"]=-8, ["y"]=448, ["shoot_func"]=grenadef, ["collection"]=grenades, ["hertz"]=0.5, ["hp"]=5, ["vx"] = 2.5, ["vy"] = 2.5, ["points"]=50},
	    {["sprite"]=104, ["x"]=8, ["y"]=368, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=50},
	    {["sprite"]=105, ["x"]=120, ["y"]=384, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = -3, ["vy"] = 0, ["points"]=50},
	    {["sprite"]=104, ["x"]=0, ["y"]=408, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=50},
      {["sprite"]=105, ["x"]=48, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = -3, ["vy"] = 0, ["points"]=50},
	    {["sprite"]=104, ["x"]=64, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 3, ["vy"] = 0, ["points"]=50},
      {["sprite"]=102, ["x"]=56, ["y"]=232, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=50},
      {["sprite"]=102, ["x"]=48, ["y"]=240, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=50},
      {["sprite"]=102, ["x"]=64, ["y"]=240, ["shoot_func"]=bulletf, ["collection"]=bullets, ["hertz"]=0.75, ["hp"]=5, ["vx"] = 0, ["vy"] = 3, ["points"]=50}
	},
    ["bosses"] = {
      {["spritedef"] = sprstates.general, ["x"] = 1, ["y"]=16, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=4, ["acc"]=1, ["updater"] = boss_update, ["hp"]=10, ["points"]=80},
      {["spritedef"] = sprstates.general, ["x"] = 112, ["y"]=30, ["shoot_func"]=grenadef, ["collection"]=grenades, ["maxspeed"]=3.5, ["acc"]=1, ["updater"] = boss_update_constant, ["hp"]=8, ["points"]=80},
      {["spritedef"] = sprstates.general, ["x"] = 2, ["y"]=30, ["shoot_func"]=grenadef, ["collection"]=grenades, ["maxspeed"]=3.5, ["acc"]=1, ["updater"] = boss_update_constant, ["hp"]=8, ["points"]=80},
      {["spritedef"] = sprstates.general, ["x"] = 8, ["y"]=34, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=3, ["acc"]=1, ["updater"] = boss_update, ["hp"]=6, ["points"]=80},
      {["spritedef"] = sprstates.general, ["x"] = 112, ["y"]=44, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=3, ["acc"]=1, ["updater"] = boss_update, ["hp"]=4, ["points"]=80},
	    {["spritedef"] = sprstates.hitler, ["x"] = 64, ["y"]=7, ["shoot_func"]=bulletf, ["collection"]=bullets, ["maxspeed"]=0, ["acc"]=0, ["updater"] = hitler_update, ["hp"]=16, ["sprh"]=2, ["points"]=150}
    }
  }
}

function update_boss(boss)
  boss.update(boss)
end

function define_bounding_box(o, x1, y1, x2, y2)
  o.bounds = {["x1"] = x1, ["x2"] = x2, ["y1"] = y1, ["y2"] = y2}
end

function update_cannon(c)
  if abs(cam_y - c.y) > 128 then
    return
  end
  c.fire = c.fire-1
  if (c.fire < 1) then
    c.shoot()
    c.fire = c.f_throttle
	add(smokes, make_smoke(c.x, c.y, c.vx / 2, c.vy / 2, 5))
  end
  local hit = false
  local hitters = {}
  local hitcheck = function(b)
    if not hit then
       hit = beenhit(c, b) and b.shooter == player
       if hit then add(hitters, b) end
    end
  end
  foreach(booms, hitcheck)
  foreach(bullets, hitcheck)
  foreach(hitters, function(b) del(bullets, b) end)
  if hit then
    c.hp = c.hp - 1
    c.flash = true
    sfx(7)
  end
  if c.hp == 0 then
    del(cannons, c)
    createbooms(c)
    sfx(6)
  end
end

function update_bullet(bullet)
 bullet.x += bullet.vx
 bullet.y += bullet.vy
 bullet.dt += bullet.vx + bullet.vy
 if bullet.dt > bullet.range then
  del(bullets, bullet)
 end
 if cmap(bullet) then
   del(bullets, bullet)
   add(smokes, make_smoke(bullet.x, bullet.y, 0 - bullet.vx, 0 - bullet.vy, 5))
   sfx(7)
 end
end

function update_grenade(g)
  g.x += g.vx
  g.y += g.vy
  if g.vx > 0 then
    g.vx -= g.deaccel
    if g.vx < 0 then g.vx = 0 end
  end
  if g.vx < 0 then
    g.vx += g.deaccel
    if g.vx > 0 then g.vx = 0 end
  end
  if g.vy > 0 then
    g.vy -= g.deaccel
    if g.vy < 0 then g.vy = 0 end
  end
  if g.vy < 0 then
    g.vy += g.deaccel
    if g.vy > 0 then g.vy = 0 end
  end
  -- stopped moving so boom
  if g.vy == 0 and g.vx == 0 then
    createbooms(g)
    sfx(6)
    del(grenades, g)
  end
end

function createbooms(g)
  for i = 0, g.boomcount do
    local boom = {}
    boom.x = g.x + (rnd(16) - 8)
    boom.y = g.y + (rnd(16) - 8)
    boom.state = sprstates.boom
    boom.sprite = boom.state[1]
    boom.tick = 1
    boom.shooter = g.shooter
    define_bounding_box(boom, -2, -2, 10, 10)
    add(booms, boom)
  end
end

function update_boom(boom)
  boom.tick += 1
  boom.sprite = boom.state[boom.tick]
  if boom.tick > #boom.state then
    del(booms, boom)
	  add(smokes, make_smoke(boom.x, boom.y))
  end
end

function update_smoke(s)
  s:update()
  if not s.alive then
	del(smokes, s)
  end
end

function soldierf(type, x, y, maxspeed, acc, hp, grenades)
 local s = {}
 s.x = x
 s.y = y
 s.type = type
 s.basesprite = sprtype[type]
 s.sprite = sprtype[type]
 s.dx = 0;
 s.dy = 0;
 s.maxspeed = maxspeed
 s.acc = acc or 0.5
 s.f_throttle = 10
 s.fire = 0
 s.tick = 1
 s.states = sprstates.soldier
 s.state = s.states.standing
 s.busy = false
 s.shoot_dir = -1
 s.hp = hp or 5
 s.points = 5
 s.grenades = grenades or game_mode.grenades
 define_bounding_box(s, 1, 2, 6, 7)
 return s
end

function kill_soldier(o)
  if o.tick > #o.state then
    if o == player then
	    lives = lives - 1
      if lives < 1 then
        _draw = game_over_draw
        _update = game_over_update
      else
        player = spawn(player)
      end
    else
      del(nazis, o)
      del(bosses, o)
    end
    if (#nazis < current_level.nazi_spawn_rate) then
      spawn_nazis()
    end
	  return
  end
  setsprite(o)
  o.tick +=1
end

function make_decision(o)
  local left = false
  local right = false
  local up = false
  local down = false
  local fire = false
  local xrand = rnd(10)
  local yrand = rnd(10)
  local frand = rnd(10)
  local decisionthresh = 3
  if xrand > decisionthresh then
    if player.x < o.x then
	    left = true
    end
    if player.x > o.x then
      right = true
    end
  end
  if yrand > decisionthresh then
    if player.y > o.y + 8 and not obstacle_in_front(o) then
      down = true
    end
    if player.y < o.y + 8 then
      up = true
    end
  end
  if abs(player.x - o.x) < 4 and frand > decisionthresh then
    fire = true
  end
  update_soldier(o, left, right, up, down, fire)
end

function jostle(n)
  local innerjostle = function(n2)
     if n2 == n then return end
     local diffx = n.x - n2.x
     local diffy = n.y - n2.y
     if abs(diffx) < 4 and abs(diffy) < 8 then
        if (diffx < 0) then n.x -= 1 else n.x +=1 end
     end
  end
  foreach(nazis, innerjostle)
end

function update_soldier(o, left, right, up, down, fire1, fire2)
  if o.busy then
    kill_soldier(o)
	return
  end
  local lx = o.x
  local ly = o.y
  local flashing = false
  if o.fire > 0 then
    o.fire -= 1
  end
  if left and o.dx > 0-o.maxspeed then
		o.dx -= o.acc
  end
  if right and o.dx < o.maxspeed then 
		o.dx += o.acc
	end
  if not left and not right then
      if o.dx < 0 then
		    o.dx += o.acc
	    elseif o.dx > 0 then
	      o.dx -= o.acc
	    end
  end
  if up and o.dy > 0 - o.maxspeed then 
		o.dy -= o.acc	
  end
  if down and o.dy < o.maxspeed then 
		o.dy += o.acc
	end
  if not up and not down then
    if o.dy < 0 then
		  o.dy += o.acc
	  elseif o.dy > 0 then
	    o.dy -= o.acc
	  end
  end
  
  o.x += o.dx
  o.y = min(o.y + o.dy, 512)
  jostle(o)
  o.tick += 1
  if cmap(o) or collide_with(o, cannons) then
    o.x = lx
	  o.y = ly
  end
  if o.state == o.states.hit and o.tick <= #o.states.hit then
    flashing = true
  end
  if not flashing then
    if o.dx == 0 and o.dy == 0 then
      o.state = o.states.standing
    else
     o.state = o.states.running
    end
  end
  if fire1 and o.fire == 0 then
    o.fire = o.f_throttle
    add(bullets, bulletf(o.x, o.y+(o.shoot_dir*8), 0, o.shoot_dir * 2, o.shoot_dir, o))
  end
  if fire2 and o.fire == 0 and o.grenades > 0 then
    o.fire = o.f_throttle
    add(grenades, grenadef(o.x, o.y+ o.shoot_dir * 4, 0, 3, o.shoot_dir, o))
    o.grenades = o.grenades - 1
  end
  if cmap(o, 1) then
    o.state = o.states.exploding
	  o.tick = 1
	  o.busy = true
    sfx(6)
	  kill_soldier(o)
	  add(smokes, make_smoke(o.x, o.y))
	  add(smokes, make_smoke(o.x, o.y))
  end
  if cmap(o, 2) then
    o.state = o.states.drowning
	  o.tick = 1
	  o.busy = true
    sfx(9)
	  kill_soldier(o)
  end
  local boomhit = false
  local boomcheck = function(b)
    if not boomhit then
       boomhit = beenhit(o, b)
    end
  end
  foreach(booms, boomcheck)
  if boomhit then
    o.state = o.states.exploding
    o.tick = 1
    o.busy = true
  end
  local h = false
  local bullcheck = function(b)
    if not h then
      h = beenhit(o, b)
      if h then
        del(bullets, b)
      end
    end
  end
  foreach(bullets, bullcheck)
  if h then
    o.hp = o.hp - 1
    if o.hp == 0 then
      o.state = o.states.shot
      o.tick = 1
      o.busy = true
      kill_soldier(o)
    else
      o.state = o.states.hit
      o.tick = 1
    end
  end
  handle_bounds(o)
  setsprite(o)
end

function setsprite(o)
  if o.tick > #o.state then
     o.tick = 1
  end
  o.sprite = o.state[o.tick]
end

function draw_thing(thing)
  if thing.flash then
    for i=1,15 do
      pal(i, 7)
    end
  end
  local height = thing.height or 1
  spr(thing.sprite, thing.x + current_level.offset, thing.y, 1, height)
  thing.flash = false
  pal()
end

function del_all(entities)
  for entity in all(entities) do
    del(entities, entity)
  end
end

function obstacle_in_front(o)
  local offset_x = current_level.offset + o.x + 2
  local x1=offset_x / 8
  local x2=(offset_x + 4) / 8
  local y=(o.y+o.bounds.y2 + 5)/8
  local obstacle = fget(mget(x1,y), 0) or fget(mget(x1,y), 1) or fget(mget(x1,y), 2) or fget(mget(x2,y), 2)
  return obstacle
end

function cmap(o, f)
  f = f or 0
  local offset_x = current_level.offset + o.x
  local ct=false
  local cb=false
  local x1=(offset_x + o.bounds.x1)/8
  local y1=(o.y + o.bounds.y1)/8
  local x2=(offset_x + o.bounds.x2)/8
  local y2=(o.y+o.bounds.y2)/8
  local ct = fget(mget(x1,y1),f) or fget(mget(x1,y2),f) or fget(mget(x2,y2),f) or fget(mget(x2,y1),f)
  return ct
end

function collide_with(o, things)
  for thing in all(things) do
    if beenhit(o, thing) then
      return true
    end
  end
  return false
end

function handle_bounds(o)
  if o.x < 0 then
    o.x = 0
  end
  if o.x > 120 then
    o.x = 120
  end
end

function beenhit(o, b)
  if o == b.shooter then
    return
  end
  local cb = false
  if o.x+o.bounds.x2 > b.x+b.bounds.x1 and 
        o.y+o.bounds.y2 > b.y+b.bounds.y1 and
        o.x+o.bounds.x1 < b.x+b.bounds.x2 and
        o.y+o.bounds.y1 < b.y+b.bounds.y2
        then
    cb = true
  end 
  if cb and b.shooter == player then
    points = min(points + o.points, 32767)
  end
  return cb
end

function spawn(oldplayer)
  if oldplayer then
    return soldierf(1,64,61*8,1.5,0.25, game_mode.hp, player.grenades)
  else
    return soldierf(1,64,61*8,1.5,0.25, game_mode.hp)
  end
end

function spawn_nazis()
  local spawny = player.y - 104
  if spawny < 8 then
    return
  end
  local nazi_spawn_total = current_level.nazi_spawn_rate + game_mode.spawn - #nazis
  for i=0,nazi_spawn_total do
    local spawnx = 16 + (i*(96 / nazi_spawn_total))
    
    local s = soldierf(5, spawnx, spawny, 0.75 + rnd(1), 0.1 + rnd(0.5), 1)
    while cmap(s) or cmap(s, 1) or cmap(s, 2) do
       s.y = s.y-4
    end
    s.states = sprstates.nazi
    s.state = s.states.standing
    s.shoot_dir = 1
    -- s.confines = {max(0, spawnx - 64), min(112, spawnx + 64)}
    add(nazis, s)
  end
end

function hoover_items(things, min, func)
  for thing in all(things) do
    if abs(thing.y - player.y) > 320 then
      del(things, thing)
      return
    end
  end
  if #things < min then
    func()
  end
end

function draw_hud()
  rectfill(2+current_level.offset, cam_y +119, 9+current_level.offset, cam_y+125, 0)
  rectfill(26+current_level.offset, cam_y +119, 33+current_level.offset, cam_y+125, 0)
  rectfill(46+current_level.offset, cam_y +119, 53+current_level.offset, cam_y+125, 0)
  spr(31, 2 + current_level.offset, cam_y + 118)
  spr(30, 26 + current_level.offset, cam_y + 119)
  spr(46, 46 + current_level.offset, cam_y + 119)
  prtext(tostr(player.hp), current_level.offset+12, cam_y + 120, 7, 0)
  prtext(tostr(player.grenades), current_level.offset+36, cam_y + 120, 7, 0)
  prtext(tostr(lives), current_level.offset+56, cam_y + 120, 7, 0)
  prtext("score: "..tostr(points), current_level.offset+70, cam_y + 120, 7, 0)
end

function update_high_scores()
  local new_score = {["name"] = "anon", ["score"] = points}
  if #high_scores == 0 then
    add(high_scores, new_score)
    return
  end
  local sorted_scores = {}
  local added_score = false
  for high_score in all(high_scores) do
    if high_score.score > new_score.score then
      add(sorted_scores, high_score)
    else
      add(sorted_scores, new_score)
      add(sorted_scores, high_score)
      added_score = true
    end
  end
  if not added_score then
    add(sorted_scores, new_score)
  end
  high_scores = sorted_scores
end

function end_level(level)
  del_all(bullets)
  del_all(grenades)
  del_all(booms)
  del_all(nazis)
  del_all(cannons)
  level += 1
  if level <= #levelstates and lives > 0 then
    init_level(level)
  else
    update_high_scores()
    _draw = draw_title
    _update = update_title
    camera(0,0)
    if lives == 0 then
      music(0, 0)
    else
      music(7, 0)
    end
  end
end

function _init()
  _update = update_title
  _draw = draw_title
  music(0, 0)
end
function _draw()
end
function _update()
end

function game_update()
  cam_y = cam_positions[1]
  del(cam_positions, cam_y)
  hoover_items(nazis, current_level.nazi_spawn_rate, spawn_nazis)
  foreach(nazis, make_decision)
  foreach(cannons, update_cannon)
  foreach(bullets, update_bullet)
  foreach(grenades, update_grenade)
  foreach(booms, update_boom)
  foreach(bosses, update_boss)
  foreach(smokes, update_smoke)
  if lives > 0 then update_soldier(player, btn(0), btn(1), btn(2), btn(3), btn(4), btn(5)) end
  local present_cam = max(min(player.y - 96, 424), 0)
  if not boss_trigger and present_cam == 0 then
    music(6)
    boss_trigger = true
  end
  add(cam_positions, present_cam)
  if player.y < -4 and #bosses == 0 then
    end_level(current_level.number)
  end
end

function debug_draw(thing)
  rect(thing.x + thing.bounds.x1, thing.y + thing.bounds.y1, thing.x + thing.bounds.x2, thing.y + thing.bounds.y2, 7)
end

function game_draw()
  cls()
  camera(cam_x, cam_y);
  map(0, 0, 0, 0, 128, 64)
  draw_thing(player)
  foreach(nazis, draw_thing)
  foreach(cannons, draw_thing)
  foreach(bullets, draw_thing)
  foreach(grenades, draw_thing)
  foreach(booms, draw_thing)
  foreach(current_level.overheads, draw_thing)
  foreach(bosses, draw_thing)
  --foreach(booms, debug_draw)
  --foreach(nazis, debug_draw)
  fillp(0b0101101001011010.1)
  foreach(smokes, function(s)
      s:draw(current_level.offset)
    end
  )
  fillp()
  draw_hud()
end

function game_over_draw()
  game_draw()
  prtext("game over! hitler keeps france.", 4 + current_level.offset, cam_y + 32, 8, 0)
  prtext("nicht sehr gut", 35 + current_level.offset, cam_y + 40, 8, 0)
end

function game_over_update()
  game_update()
  if btnp(4) then
    end_level(current_level.number)
  end
end

function init_level(level)
  current_level = levelstates[level]
  player.y = 488
  cam_x = current_level.offset
  cam_y = 0
  cam_positions = {392, 392, 392, 392, 392, 392, 392}
  spawn_nazis()
  cannons = {}
  bosses = {}
  foreach(current_level.cannons, function(c) add(cannons, cannonf(c)) end)
  foreach(current_level.bosses, function(b) add(bosses, bossf(b)) end)
  music(0)
  boss_trigger = false
end

function prtext(text, x, y, colour, back)
 back = back or 0
 for x_ = -1,1 do
   for y_ = -1,1 do
      print(text, x + x_, y + y_, back)
   end
 end
 print(text, x, y, colour)
end

function draw_title()
  cls()
  fillp(0b0101101001011010.1)
  foreach(booms, draw_thing)
  foreach(smokes, function(s)
      s:draw(current_level.offset)
    end
  )
  fillp()
  prtext("tuesday 6th june, 1944", 20, 24, 7, 3)
  prtext("d-day", 54, 32, 7, 3)
  prtext("press fire to start", 26, 40, 7, 3)
  local line = 56
  for gm in all(game_modes) do
    if cheat_mode < 5 and gm == game_modes[5] then
      break
    else
      if gm == game_mode then
        prtext(">"..gm.name.."<", 44, line, 7, 3)
      else prtext(gm.name, 44, line, 8, 1)
      end
      line += 8
    end
  end
  prtext("high score", 44, line + 8, 7, 3)
  if #high_scores == 0 then
    prtext("---- ----", 44, line + 16, 7, 3)
  else
    prtext(tostr(high_scores[1].score), 44, line + 16, 7, 3)  
  end

end

function update_title()
  current_level = {["offset"] = 0}
  foreach(smokes, update_smoke)
  foreach(booms, update_boom)
  if #booms <20 then
    createbooms({["x"] = rnd(128), ["y"] = rnd(128), ["boomcount"] = 2})
  end
  local gmcount = 1
  local gmindex
  local cmhack = 1
  if cheat_mode > 4 then
    cmhack = 0
  end
  for gm in all(game_modes) do
    if gm == game_mode then
      gmindex = gmcount
    end
    gmcount +=1
  end
  if btnp(2) and gmindex > 1 then
    game_mode = game_modes[gmindex-1] 
  end
  if btnp(3) and gmindex < #game_modes - cmhack then
    game_mode = game_modes[gmindex+1]
  end
  if btnp(4) then
    _update = game_update
    _draw = game_draw
    lives = game_mode.lives
    points = 0
    player = spawn()
    init_level(1)
  end
  if btnp(5) then
    cheat_mode += 1
  end
end

__gfx__

__gff__

__map__

__sfx__

__music__
00 01434144
00 01434144
00 02434244
00 02434244
00 03414144
02 03414144
03 04414144
03 08414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144
00 41414144

